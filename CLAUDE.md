...
## Tärkeät tiedostot
- `start.py` - **Tuotannon Railway-entrypoint**. Ajaa `alembic upgrade head` ennen Uvicornia ja sen jälkeen `app.main:app`:in. EI `main.py` suoraan. Escape hatch: `SKIP_ALEMBIC_UPGRADE=1` vain rollback-/hätätilanteisiin.
...
- `app/auth/` - **Canonical Platform Identity v2** (2026-05-14, laajennettu 2026-05-15): `CanonicalUser`, `CanonicalTokenError`, `create_canonical_token`, `decode_canonical_token`, `provision_canonical_user`, `get_current_canonical_user`. Token shape `{sub=<uuid>, email, org_id, role, product, jti, iat, exp}` HS256. `product` normalisoidaan allowlististä (`veyra`, `continuity`, `growth_engine`, `kirjanpito`, `jobscout`, `bemufix`; fallback `unknown`) ja toimii Phase 4.2 Profile Facts API:n anti-spoofing-prerequisitena.
...
- `app/auth/apple.py` - Apple Sign In verifier (python-jose + JWKS cache). Varmistaa RS256-signaturen, issuerin, exp:n ja bundle audience-listan (`APPLE_BUNDLE_IDS`). Ei luota client-forwarded emailiin identity-resoluutiossa.
- `app/routers/auth_v2.py` - 8 endpointtia: `/me`, `/logout`, `/google/native`, `/apple/native`, `/google/login` + `/callback` (OAuth-redirect web-frontendille), `/magic-link/request`, `/magic-link/verify`. Kaikki v2-issuance-reitit lukevat `X-Brandista-Product`-headerin ja leipovat normalisoidun `product`-claimin tokeniin; Google web callback hardcodaa `growth_engine`.
- `migrations/` - Alembic. Revisioketju: `0001_baseline` (legacy-tilan stampaus) → `0002_canonical_id` (uudet taulut + users in-place) → `0003_pwd_nullable` (drop NOT NULL hashed_password:lta) → `0004_apple_id` (`users.apple_id`) → `0005_drop_redundant_idx` → `0006_profile_facts` (Phase 4.2, työpuussa). `start.py` ajaa `alembic upgrade head` automaattisesti Railway-bootissa.
...
- **2026-05-15**: Phase 4.1 step 3.5 Apple Sign In canonical-kerrokseen tuotantoon. `0004_apple_id` + Apple JWKS verifier + `/api/auth/v2/apple/native`; tuotanto-incidentti (migraatio ei ajanut bootissa) korjattu pysyvästi `start.py` alembic-on-bootilla. cubic-dev-ai:n löytämä P0 account-takeover -riski korjattu: client-forwarded email on informational-only.
- **2026-05-15**: Phase 4.2 step 1 `product` JWT claim retrofit merged. 7 uutta auth-v2-yksikkötestiä. Profile Facts API speksattu `docs/superpowers/specs/2026-05-15-phase-4-2-facts-api-design.md`.
- **2026-05-15 iltapäivä**: Phase 4.2 steps 2–4 end-to-end tuotantoon. **Toteutusaika 1 päivä (alkuperäinen arvio 3–4 viikkoa).**
  - **Step 2** — `0006_profile_facts` taulu, `ProfileFact` SQLAlchemy-malli, `app/schemas/facts.py` (FactCreate/Fact/FactList, key-regex `^[a-z][a-z0-9_]{0,118}[a-z0-9]$`), `app/auth/facts_safety.py` (dose-patternit + raw-diagnoosi-keys ja value-content-skannaus rekursiivisesti), `app/routers/facts.py` (POST/GET/DELETE/bulk-DELETE `/api/v1/profile/facts`). 36 yksikkötestiä.
  - **Step 2.5** — Safety-scope confidence-policy: `scope=safety` + `confidence=high` vain Continuitylle, muut productit voivat kirjoittaa `medium`/`low` ilman medical-päätösten ohitusta. Tämä avaa Veyran LIM_ADD-rajoitusten cross-product-näkyvyyden ilman SBE-impactia.
  - **Step 3** — Audience-pohjainen `product`-resolution (post-cubic-P1): Veyra-token ei voi enää spoofata `X-Brandista-Product: continuity`. `PRODUCT_AUDIENCE_MAP` env-pohjainen aud→product-mappaus (Google client_id / Apple bundle_id → canonical product-tag).
  - **Step 4** — `/api/v1/internal/profile/facts` (read-only server-to-server), `BRANDISTA_INTERNAL_SECRET` env-pohjainen auth constant-time-vertailulla, fail-loud 503 unconfigured-tilassa. POST/DELETE EI exposed internal-prefixillä (blast radius read-only).
  - **Audit-päivitys**: `0006_profile_facts` aiemmin "työpuussa" → nyt tuotantoinformaatiossa, alembic-version `0006_profile_facts`, smoke-test 6/6 matrix-tapausta + cross-product Railway-lokirivinä `GET /api/v1/internal/profile/facts ... HTTP/1.1 200 OK`.
- **2026-05-15 ilta**: Phase 4.3 event-bus spec valmis `docs/superpowers/specs/2026-05-15-phase-4-3-event-bus-design.md`. Outbox-pattern ilman ulkoista brokeria, 4 event-tyyppiä, HMAC-allekirjoitukset, replay-CLI, ~10 työpäivän ship-sekvenssi. Karoliinan use case (HRV drop → coach plan softens) = definition of done.
