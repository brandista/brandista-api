...
async def analyze_basic_metrics_enhanced(url: str, html: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if url.startswith('https://'):
            ...
            details['https'] = True
            security_headers = check_security_headers(html)
            if security_headers['csp']: score_components['security'] += 2
            if security_headers['x_frame_options']: score_components['security'] += 1
            if security_headers['strict_transport']: score_components['security'] += 2
        else:
            details['https'] = False
        ...
        # CONTENT using configurable scoring
...
async def analyze_technical_aspects(url: str, html: str) -> Dict[str, Any]:
    ...
    if meta_desc:
        ...
        elif 80 <= L <= 200: meta_score += 4
        elif L > 0: meta_score += 2
    tech_score += meta_score

    # Page speed estimation
    size = len(html)
    if size < 50_000: tech_score += 5
    elif size < 100_000: tech_score += 4
    elif size < 200_000: tech_score += 2
    elif size < 500_000: tech_score += 1
    ...
    return {
        'has_ssl': has_ssl, 'has_mobile_optimization': has_mobile,
        'page_speed_score': 75, 'has_analytics': analytics['has_analytics'],
        'has_sitemap': check_sitemap_indicators(soup), 'has_robots_txt': check_robots_indicators(html),
        'meta_tags_score': meta_score, 'overall_technical_score': final,
        'security_headers': sh, 'performance_indicators': []
    }
...
async def analyze_content_quality(html: str) -> Dict[str, Any]:
    ...
    # Volume using configurable scoring
...
async def analyze_social_media_presence(url: str, html: str) -> Dict[str, Any]:
    ...
    score = len(platforms) * 10  # Base score per platform
...
async def analyze_competitive_positioning(url: str, basic: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if score >= 75:
        ...
        advantages = ["Excellent digital presence", "Advanced technical execution"]
        threats = ["Pressure to innovate continuously"]
        comp_score = 85
    elif score >= 60:
        position = "Strong Performer"
        advantages = ["Solid digital foundation"]
        threats = ["Gap to market leaders"]
        comp_score = 70
    elif score >= 45:
        position = "Average Competitor"
        advantages = ["Baseline established"]
        threats = ["At risk of falling behind"]
        comp_score = 50
    else:
        position = "Below Average"
        advantages = ["Significant upside potential"]
        threats = ["Competitive disadvantage"]
        comp_score = 30
    ...
    return {
        ...
        'industry_comparison': {
            'your_score': score, 'industry_average': 45,
            'top_quartile': 70, 'bottom_quartile': 30
        }
    }
...
def generate_english_insights(overall: int, basic: Dict[str, Any], technical: Dict[str, Any], content: Dict[str, Any], ux: Dict[str, Any], social: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if overall < 30:
        opportunities.extend([
            f"Massive upside - target {overall + 40} points",
            "Fundamentals can yield +20-30 points quickly"
        ])
    elif overall < 50:
        opportunities.extend([
            f"Growth potential - target {overall + 30} points",
...
async def ai_analyze_comprehensive(
    ...
    try:
        ...
        cache_key = get_cache_key(url, "ai_comprehensive_v6")
        ...
        # Run all analyses
        basic_analysis = await analyze_basic_metrics_enhanced(url, html_content)
        technical_audit = await analyze_technical_aspects(url, html_content)
...
async def basic_analyze(request: CompetitorAnalysisRequest, user: UserInfo = Depends(require_user)):
    try:
        ...
        basic_analysis = await analyze_basic_metrics_enhanced(url, response.text)
...
def detect_technology_stack(html: str, soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    return {
        "detected": detected, "count": len(detected),
        "categories": {
...
def estimate_core_web_vitals(soup: BeautifulSoup, html: str) -> Dict[str, Any]:
    ...
    if cls_status != "Good":
        recs.append("Add explicit width/height to images and embeds")
...
def estimate_traffic_rank(url: str, basic: Dict[str, Any]) -> str:
    ...
    elif s >= 60: return "Top 25% in industry (Good traffic potential)"
    elif s >= 45: return "Average (Moderate traffic)"
    elif s >= 30: return "Below average (Limited traffic)"
    else: return "Low visibility (Minimal traffic)"
...
def calculate_improvement_potential(basic: Dict[str, Any]) -> int:
    ...
    for cat, max_pts in SCORING_CONFIG.weights.items():
...
def generate_competitor_gaps(basic: Dict[str, Any], competitive: Dict[str, Any]) -> List[str]:
    s = basic.get('digital_maturity_score', 0)
    if s < 30:
        return ["Very weak digital presence vs. peers", "Foundational optimizations missing"]
    elif s < 50:
        return ["Content strategy lags behind peers", "Technical implementation below average"]
    elif s < 70:
        return ["Gap to top performers remains", "Potential to catch up with focused investments"]
    else:
        return ["Competitive vs. most peers", "Focus on innovation to differentiate"]
...
if __name__ == "__main__":
    ...
    # Admin password security: don't log it at all
    ...
    if "localhost" not in str(ALLOWED_ORIGINS) and "*" in ALLOWED_ORIGINS:
        logger.warning("⚠️  CORS allows all origins (*) - restrict ALLOWED_ORIGINS in production!")
